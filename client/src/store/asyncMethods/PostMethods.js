...
export const updateAction = (updatedPost) => {
    return async (dispatch, getState) => {
        ...
        dispatch({ type: SET_LOADER });
        try {
            const { data } = await axios.post('/editpost', updatedPost, config);
            dispatch({ type: CLOSE_LOADER });
            dispatch({ type: REDIRECT_TRUE });
...
export const updateImageAction = (updatedImage) => {
    return async (dispatch, getState) => {
        ...
        dispatch({ type: SET_LOADER });
        try {
            const { data: {msg} } = await axios.post('/updateImage', updatedImage, config);
            dispatch({ type: CLOSE_LOADER });
            dispatch({type: REDIRECT_TRUE});
            dispatch({type: SET_MESSAGE, payload: msg})
        } catch (error) {
...
export const homeTestingPosts = () => {
    return async(dispatch) => {
        dispatch({type: SET_LOADER});
        try{
            const{data} = await axios.get("/homeTesting");
            console.log(data);
...
export const postDetailView = (id) => {
    return async(dispatch) => {
        dispatch({type: SET_LOADER});
        try{
            const{data: {post}} = await axios.get(`/postview/${id}`);
